...
<body>
    ...
    <section class="admin-list">
      <div class="form">
        <form action="update.php" method="post" enctype="multipart/form-data" class="add-form">
          <div class="row-1">
            <div class="input-container">
              <label for="recipe_name">Nombre</label>
              <input name="recipe_name" class="input-name" type="text" id="recipename" value="<?php echo $data[0]["recipe_name"];?>">
            </div>
            ...
            <div class="input-container">
              ...
              <select name="preparation_time" class="selector" id="preparationtime">
                <?php
                  ...
                  for($i=0; $i <$len; $i++){
                    if($data[0]["preparation_time"] == $prep_time[$i]){
                        echo '<option value="'.$prep_time[$i].'" selected>'.$prep_time[$i].'</option>';
                    }else{
            ...
            <div class="input-container">
              ...
              <?php
                  ...
                  for($i=0; $i <$len; $i++){
                    if($data[0]["cooking_time"] == $cook_time[$i]){
                        echo '<option value="'.$cook_time[$i].'" selected>'.$cook_time[$i].'</option>';
                    }else{
            ...
            <div class="input-container">
              ...
              <?php
                  ...
                  for($i=0; $i <$len; $i++){
                    if($data[0]["total_time"] == $total_time[$i]){
                        echo '<option value="'.$total_time[$i].'" selected>'.$total_time[$i].'</option>';
                    }else{
            ...
            <div class="input-container">
              ...
              <?php
                  ...
                  for($i=0; $i <$len; $i++){
                    if($data[0]["portions"] == $portions[$i]){
                        echo '<option value="'.$portions[$i].'" selected>'.$portions[$i].'</option>';
                    }else{
                ...
                <?php
                  ...
                  for($i=0; $i <$len; $i++){
                    if($data[0]["id_recipe_complex"] == $complex[$i]['id_recipe_complex']){
                        echo '<option value="'.$complex[$i]['id_recipe_complex'].'" selected>'.$complex[$i]['complex_name'].'</option>';
                    }else{
                ...
                <?php
                  ...
                  for($i=0; $i <$len; $i++){
                    if($data[0]["id_recipe_category"] == $categories[$i]['id_recipe_category']){
                        echo '<option value="'.$categories[$i]['id_recipe_category'].'" selected>'.$categories[$i]['category_name'].'</option>';
                    }else{
                ...
                <?php
                  ...
                  for($i=0; $i <$len; $i++){
                    if($data[0]["id_recipe_occasion"] == $occasions[$i]['id_recipe_occasion']){
                        echo '<option value="'.$occasions[$i]['id_recipe_occasion'].'" selected>'.$occasions[$i]['occasion_name'].'</option>';
                    }else{
          ...
          <div class="row-1">
            <div class="input-container">
              ...
              <textarea name="recipe_instructions" class="input-txta" id="instructions" cols="30" rows="10"><?php echo $data[0]["recipe_instructions"]; ?></textarea>
            </div>

            <div class="input-container">
              <label for="recipe_description">Descripción</label>
              <textarea name="recipe_description" class="input-txta" id="description" cols="30" rows="10"><?php echo $data[0]["recipe_description"]; ?></textarea>
            </div>

            <div class="input-container">
              <label for="recipe_ingredients">Ingredientes</label>
              <textarea name="recipe_ingredients" class="input-txta" id="ingredients" cols="30" rows="10"><?php echo $data[0]["recipe_ingredients"]; ?></textarea>
            </div>

            <div class="input-container">
              <img id="preview" src="../img/<?php echo $data[0]["image_name"]; ?>" width="200" height="200" alt="Preview">
          ...
          <div class="row-1">
            ...
            <input name="id_recipe" type="hidden" value="<?php echo $data[0]["id_recipe"];?>"> <!--Prestar Atención-->
